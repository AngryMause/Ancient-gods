...
class GameRepository(
    ...
    private suspend fun playWinSound() {
        val isEnabled = appOptionPreferences.isSoundEnabled().first()
        if (!isEnabled) return
        audioPlayer.playWinSound()
    }

    private suspend fun playAndGameSound() {
        val isEnabled = appOptionPreferences.isSoundEnabled().first()
        if (!isEnabled) return
//        audioPlayer.playAndGameSound()
    }
    ...
    suspend fun onSkillClicked(skillsBarEvent: SkillsBarEvent) {
        when (skillsBarEvent) {
            SkillsBarEvent.THUNDER -> {
                _monsterListFlow.value.forEachIndexed { index, monsterModel ->
                    if (monsterModel.health > 0 && monsterModel.offset.y < screenSize.height - 300) {
                        val new = monsterModel.copy(health = 0)
                        if (new.health == 0) {
                            _topBarModelFlow.emit(
                                _topBarModelFlow.value.copy(
                                    points = _topBarModelFlow.value.points + 2,
